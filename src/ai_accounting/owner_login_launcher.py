"""Launch the local owner's password prompt when an MCP session needs renewal.

The launcher is intentionally Windows-only.  It passes only the configured login
name to the existing identity CLI; passwords remain no-echo interactive input and
the resulting opaque session token remains in Windows Credential Manager.
"""
