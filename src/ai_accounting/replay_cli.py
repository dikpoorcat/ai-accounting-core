...
def _write_json(path: Path, value: Any, *, compact: bool = False) -> None:
    # Replace only a complete checkpoint; a killed replay must leave readable state.
    payload = (
        json.dumps(
            _jsonable(value),
            ensure_ascii=False,
...
@contextmanager
def _replay_state_lock(state_path: Path) -> Iterator[None]:
    """OS-owned lock survives state replacement and is released on process exit."""
    ...
    with lock_path.open("a+b") as stream:
        if os.name == "nt":
            ...
            try:
                ...
                raise ReplayError("REPLAY_STATE_ALREADY_IN_USE") from None
        else:
            import fcntl
...
_REFERENCE_FIELDS: dict[str, frozenset[str]] = {
    ...
    "component_entity": frozenset({"source_replay_key", "component_key", "entity_kind"}),
    "component_line": frozenset(
        {"source_replay_key", "component_key", "line_kind", "identity_code"}
    ),
    "joined_key": frozenset({"parts"}),
...
def _stable_maps(session: Session, org_id: uuid.UUID) -> dict[str, dict[str, Any]]:
    ...
    # Card codes and calculated payroll/labor line IDs can be generated by the
    # target database. Bind them to the originating component, never the old UUID.
    for table, kind in (
        ("fixed_assets", "asset"),
        ...
        for row in _query_rows(
            session,
            f"SELECT entity.id, component.key, event.idempotency_key FROM {table} entity "
            "JOIN business_event_components component ON component.org_id=entity.org_id "
            "AND component.id=entity.component_id JOIN business_events event "
            "ON event.org_id=component.org_id AND event.id=component.event_id "
            "WHERE entity.org_id=:org_id",
            org_id=org_id,
        ):
            maps[kind][str(row["id"])] = {
                "$ref": "component_entity",
                "entity_kind": kind,
                "source_replay_key": _semantic_replay_key(str(row["idempotency_key"])),
                "component_key": row["key"],
            }
    line_identities: dict[str, tuple[str, str]] = {}
    for table, identity_table, identity_column, code_column, kind in (
        ("payroll_lines", "employees", "employee_id", "employee_code", "payroll"),
        (
        ...
        for row in _query_rows(
            session,
            f"SELECT line.id, identity.{code_column} AS code FROM {table} line "
            f"JOIN {identity_table} identity ON identity.org_id=line.org_id "
            f"AND identity.id=line.{identity_column} WHERE line.org_id=:org_id",
            org_id=org_id,
        ):
            line_identities[str(row["id"])] = (kind, str(row["code"]))
    for reference in maps["open_item"].values():
        if reference["$ref"] != "component_open_item":
            continue
        key = str(reference["open_item_key"])
        if not _UUID_TEXT.search(key):
            continue
        parts: list[Any] = []
        position = 0
        for match in _UUID_TEXT.finditer(key):
            identity = line_identities.get(match.group())
            if identity is None:
                raise ReplayError("REPLAY_OPEN_ITEM_KEY_IDENTITY_UNKNOWN")
            parts.extend(
                [
                    key[position : match.start()],
                    {
                        "$ref": "component_line",
                        "source_replay_key": reference["source_replay_key"],
                        "component_key": reference["component_key"],
                        "line_kind": identity[0],
        ...
        reference["open_item_key"] = {"$ref": "joined_key", "parts": parts}
    return maps
...
def _event_operation(
    ...
    *,
    ...
    if event_type in {"fixed_asset_depreciation", "fixed_asset_depreciation_batch"}:
        request = {
            "org_id": "${ORG_ID}",
...
def _owner_control_operations(
    ...
    *,
    ...
    for row in period_rows:
        ...
        plan = (row["source_snapshot"] or {}).get("regular_payroll_plan")
        payroll_items = (
            _replace_stable_references(plan["employee_items"], org_id=org_id, maps=maps)
...
class _ReplayResolver:
    ...
    def materialize(self, value: Any) -> Any:
        ...
        if ref_type == "joined_key":
            return "".join(str(self.materialize(item)) for item in value["parts"])
            ...
            if ref_type == "evidence":
                ...
                result = session.execute(
                    ...
                    ),
                    {
                        ...
                        "event_id": event_id,
                        "component_key": value["component_key"],
                        "open_item_key": self.materialize(value["open_item_key"]),
                    },
                ).scalar_one_or_none()
            elif ref_type in {"component_entity", "component_line"}:
                source = self.results.get(str(value["source_replay_key"]))
                event_id = source.get("event_id") if source else None
                if event_id is None:
                    raise ReplayError("REPLAY_COMPONENT_SOURCE_MISSING")
                ...
                if ref_type == "component_entity":
                    table = {
                        "asset": "fixed_assets",
                        "intangible": "intangible_assets",
                        "borrowing": "borrowings",
                    }.get(str(value["entity_kind"]))
                    if table is None:
                        raise ReplayError("REPLAY_COMPONENT_ENTITY_KIND_INVALID")
                    sql = (
                        f"SELECT entity.id FROM {table} entity JOIN business_event_components c "
                        "ON c.org_id=entity.org_id AND c.id=entity.component_id "
                        "WHERE c.org_id=:org_id AND c.event_id=:event_id AND c.key=:component_key"
                    )
                else:
                    parameters["code"] = value["identity_code"]
                    if value["line_kind"] == "payroll":
                        sql = (
                            "SELECT DISTINCT line.id FROM payroll_event_links link "
                            "JOIN payroll_lines line ON line.org_id=link.org_id "
                            "AND line.payroll_batch_id=link.payroll_batch_id "
                            "JOIN employees identity ON identity.org_id=line.org_id "
                            "AND identity.id=line.employee_id "
                            "JOIN business_event_components c ON c.org_id=link.org_id "
                            "AND c.id=link.component_id WHERE c.org_id=:org_id "
                            "AND c.event_id=:event_id AND c.key=:component_key "
                            "AND identity.employee_code=:code"
                        )
                    elif value["line_kind"] == "labor":
                        sql = (
                            "SELECT DISTINCT line.id FROM labor_remuneration_event_links link "
                            "JOIN labor_remuneration_lines line ON line.org_id=link.org_id "
                            "AND line.batch_id=link.batch_id "
                            "AND (link.labor_line_id IS NULL OR line.id=link.labor_line_id) "
                            "JOIN labor_service_persons identity "
                            "ON identity.org_id=line.org_id AND identity.id=line.labor_person_id "
                            "JOIN business_event_components c ON c.org_id=link.org_id "
                            "AND c.id=link.component_id WHERE c.org_id=:org_id "
                            "AND c.event_id=:event_id AND c.key=:component_key "
                            "AND identity.person_code=:code"
                        )
                    else:
                        raise ReplayError("REPLAY_COMPONENT_LINE_KIND_INVALID")
                result = session.execute(text(sql), parameters).scalar_one_or_none()
...
def replay_system(
    ...
    *,
    ...
    package_root = package.resolve(strict=True)
    path = (state_path or _default_state_path(package_root)).resolve()
    with _replay_state_lock(path):
        return _replay_system_locked(package_root, path, progress=progress)


def _replay_system_locked(
    package: Path,
    state_path: Path,
    *,
    ...
    timing: dict[str, Any] = {
        ...
        "operation_seconds_by_kind": {},
        "checkpoint_seconds": 0.0,
        "completed_operations": 0,
    }
    state["last_run_timing"] = timing
        ...
        try:
            ...
            for operation in operations:
                ...
                progress_data = {
                    ...
                    "operation_key": key,
                    "kind": operation["kind"],
                    "company_total": len(operations),
                    "company_completed": len(completed),
                }
                if progress is not None:
                    progress({"status": "operation_started", **progress_data})
                ...
                kind_timing = timing["operation_seconds_by_kind"].setdefault(
                    str(operation["kind"]), {"count": 0, "seconds": 0.0}
                )
                kind_timing["count"] += 1
                kind_timing["seconds"] += operation_seconds
                timing["completed_operations"] = completed_total
                timing["elapsed_seconds"] = time.perf_counter() - run_started
                checkpoint_started = time.perf_counter()
                _write_json(state_file, state, compact=True)
...
def verify_replay(package: Path, state_path: Path | None = None) -> dict[str, Any]:
    ...
    path = (state_path or _default_state_path(package_root)).resolve()
    with _replay_state_lock(path):
        return _verify_replay_locked(package_root, path)


def _verify_replay_locked(package: Path, state_path: Path) -> dict[str, Any]:
    package_root = package.resolve(strict=True)
    state_file, state = _load_state(package_root, state_path)
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    replay.add_argument("--quiet", action="store_true", help="omit per-operation stderr progress")
