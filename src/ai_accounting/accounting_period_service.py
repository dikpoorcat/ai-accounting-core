...
class AccountingPeriodService:
    ...
    def _assistant_review_checklist(
        ...
        *,
        ...
        payroll_payment_event_count = (
            self.session.scalar(
                select(func.count(func.distinct(PayrollEventLink.event_id)))
                .join(BusinessEvent, BusinessEvent.id == PayrollEventLink.event_id)
                .where(
        ...
        declaration_rows = self.session.execute(
            select(LaborRemunerationLine, BusinessEventComponent)
            .join(
                LaborRemunerationEventLink,
        ...
        gross_without_withholding_rows = [
            (line, component)
            for line, component in declaration_rows
            if component.facts["settlement_mode"] == "gross_paid_without_withholding"
        ...
        labor_payout_count = sum(
            ...
            <= date.fromisoformat(component.derived["payment_date"])
            <= period.end_date
            for _, component in declaration_rows
        )
        ...
        for labor_line, component in declaration_rows:
            if (
                component.facts["settlement_mode"] == "gross_paid_without_withholding"
                or labor_line.external_declaration_status == "confirmed"
            ):
                continue
            payment_date = date.fromisoformat(component.derived["payment_date"])
            due_date = (
                date(payment_date.year + 1, 1, 15)
                if payment_date.month == 12
                else date(payment_date.year, payment_date.month + 1, 15)
            )
            ...
            },
            {
                ...
                "system_facts": {
                    ...
                    "gross_paid_without_withholding_theoretical_tax_fen": sum(
                        component.derived["theoretical_withholding_tax_fen"]
                        for _, component in gross_without_withholding_rows
                    ),
            ...
            sources.append(
                {
                    ...
                    "request_payload_hash_at_close": (
                        self.session.scalar(
                            ...
                            .order_by(BusinessEventAmendment.revision.desc())
                            .limit(1)
                        )
    ...
    def _module_checks(
        ...
        no_payroll_employee_ids = self._confirmed_no_payroll_employee_ids(org_id, period)
        ...
        unfinished_labor = int(unfinished_labor_batches)
        return {
            "fixed_assets": {
