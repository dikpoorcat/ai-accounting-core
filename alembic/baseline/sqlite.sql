...
CREATE TABLE open_items (
	...
	FOREIGN KEY(org_id) REFERENCES organizations (id) ON DELETE CASCADE,
	CONSTRAINT ck_open_item_pass_through_metadata CHECK ((payable_category IS NOT NULL AND payable_category = 'pass_through' AND pass_through_key IS NOT NULL AND pass_through_beneficiary_id IS NOT NULL) OR ((payable_category IS NULL OR payable_category <> 'pass_through') AND pass_through_key IS NULL AND pass_through_beneficiary_id IS NULL)),
...
CREATE UNIQUE INDEX uq_payroll_event_link_payment_source ON payroll_event_links (org_id, component_id, link_kind, source_payment_event_id, source_open_item_id) WHERE source_payment_event_id IS NOT NULL AND source_open_item_id IS NOT NULL;
