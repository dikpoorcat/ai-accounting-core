...
> 当前版本是单企业私有试用内核，不是法定账簿、自动纳税申报或税务意见。季度财务报表功能只生成负责人复核和手工导入电子税务局的申报准备文件，不自动登录、提交或替代负责人确认。历史试用方案曾包含离线专业校准；是否沿用由用户在启动相应步骤时重新决定。
...
- 严格参数、逐次认证的本地 STDIO MCP 工具；无聊天 UI 或内置模型调用。本地经营概览另提供只读预览和 XLSX 下载路由，只监听环回地址。
...
`http://127.0.0.1:8765/`。工作台将“经营简报”和“财务报表”作为两个独立模块：经营简报
支持切换已生成月份、查看业务闭环、复式凭证、关联凭据文件名和待结事项；财务报表支持
按季度核对和展开三表。
只有报表状态为 `calculated` 且所有勾稽检查通过时才显示可用下载；缺少事实时下载路由
返回 HTTP 409 和具体阻断项。页面及下载过程都不写数据库，服务也不监听外部网络；停止
服务可在启动窗口按 `Ctrl+C`。

如果找不到原启动窗口，或修改页面后需要重启，可在仓库目录运行：

```powershell
.\deploy\windows\restart_overview.ps1 -OpenBrowser
```

脚本只会停止确认由本仓库启动、且正在监听 `8765` 的经营概览进程，然后在后台重新启动；
...
4. 在本地经营概览核对三表后，导出已填充的电子税务局 Excel 导入文件 `财务报表报送与信息采集（小企业会计准则）月季报_YYYYQn.xlsx`。导出器只复制内置固定模板并写入指定单元格，不在服务器落地临时申报文件。
