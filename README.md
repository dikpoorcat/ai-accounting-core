...
只说“开始记账”时，AI 调用只读 `finance_get_owner_brief` 汇报公司、开放期间、未匹配流水、迟到银行证据、应收应付和最近入账。该摘要只表示内核已经保存的事实，始终返回外部资料完整性尚未建立，不能据此宣称没有其他业务。直接提供具体任务或资料时，AI 选择公司后直接处理，不先复述经营看板。
