...
泛化“开始记账”调用 `workflow(as_of=实际查询日)`，默认选最早真实待处理月，展示银行、工资、普通业务、税务、资产和融资的完整状态；明确交办某件事时直接处理。`period_readiness` 查看指定月份。没有可靠月份的空公司返回空状态；未办理申报和文件任务独立保留，不把核算月份拉回闭月。先查已有资料再问，负责人回答只覆盖刚说明的公司、月份和事项。

`external_completion` 保存真实办理及原依据，`external_basis_review` 单独保存与正式账务的核对。可以先登记报税、社保办理，再核对账务；季度税务与季度财报分开，财报须等待对应期间关闭。已申报、账务待核对、文件未生成可以同时成立。

响应丢失时重发原请求和原请求键；只知道请求编号时用 `request_result` 读取原回执。`unknown` 只表示尚未观察到提交，不能当作失败另记一笔。任务返回稳定的 `error_code` 和直白说明，先处理失败原因，再重试原任务。切公司或中断后重新读取公司上下文和工作状态。
...
- CSV/XLS/XLSX 逐项原始资料核对，包含隐藏数据、金额、期间、控制合计和拆分；PDF、图片与文本保存页码或位置依据。关账、期间准备和完整代发共用检查：闭期资料问题由后续开放月持续承接，未来行到所属月才阻断，未知期间必须补足依据。清单计数不能代替逐项处置。
- 公司说明防覆盖更新；统一对象登记与版本档案，按业务类型、对象、期间、状态分页找回原始事实和来源。身份纠错、业务替代及重复核对保留不可变依据，当前归属与当时冻结分别读取。
- 普通和 bounded 工资正式发布都核对本月确认：采用附证据的明确方案，或完整人员范围的明确无变化确认。确认绑定工资输入、档案、实际采用的政策修订及变更通知。`confirm_payroll_preparation` 只登记工资事实，发布仍走统一入口；累计或实际扣税变化不因重算本身要求重复确认。
- 工资准备和期间待办提前检查个税文件的月度扣款映射。映射缺失、错误或模板不能表达只影响个税文件，不阻断合法工资、付款、关账或真实外部申报完成。导出仍复验正式结果、人员资料和扣除明细，不猜测险种。
- 工资、奖金、劳务、税务与申报日历共用政策来源校验：仅接受合法 HTTPS 官方 `gov.cn` 域名及子域，不接受凭据或伪装主机；保存来源原文，不在计算时联网，不改变税率及生效期。
...
当前开发基线支持 `restore_company` 恢复相同结构的格式 2 公司包，`verify_integrity` 核验保存内容、正式发布链、关账采用、投影和引用目录；`rebuild` 维修当前金额、期间余额和清偿贡献投影，`repair_read_indexes` 从冻结原文维修引用目录。两类维修先核验权威来源，实际变化才增加读取修复版本，不改历史事实、发布链与关账。关账直接记录实际采用的版本与依据，缺失依据明确拒绝，不提供旧格式推断或有限核验。
