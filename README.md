...
只说“开始记账”时，AI 先调用只读 `finance_get_owner_brief` 获取背景，再调用 `finance_get_owner_workflow` 读取内核派生的权威九项流程。摘要只表示内核已经保存的事实，不能据此宣称没有其他业务；清单的符号、期限、完成依据、当前动作和关账门禁只能采用流程工具返回值，不能由聊天记忆或 AI 自行打勾。老板对“无人员变化”“材料已齐”“已申报”等事实作答后，AI 必须调用对应的类型化确认工具持久化，并重新读取流程；相关员工、工资、政策、实际申报或材料快照发生变化时，旧确认自动过期并重新打开步骤。开场最后展示一个可立即执行的“下一步”和完整九项“待办清单”，以后每一次出现“下一步：”时，其正下方也必须附上更新后的完整清单。每行只显示一个状态符号：`✅` 已完成、`🔄` 当前、`⏰` 到期或逾期、`⬜` 待办、`➖` 本期无。固定顺序为：银行流水；员工及工资变动；社保及公积金；个人所得税；票据及非银行业务；关账确认；税费申报及财务报表；企业所得税年度汇算清缴；工商年报。第 7 项依赖第 6 项关账，不再反向阻塞关账；年度汇算和工商年报一旦产生会跨月持续显示，直至确认完成。负责人确认内核启用前第 7—9 项全部适用历史义务已完成、但实际完成日期没有留存时，AI 使用流程返回的截止候选调用专用历史追认工具，保存“负责人确认完成、具体日期未建立”的类型化事实；不得把法定截止日、确认当天或猜测日期写成实际申报日，也不得借此提前确认当前或未来义务。相邻步骤不得合并成一个大问题，只列负责人需要提供、确认或在外部办理的事项。上月工资和单位社保公积金计提属于关账义务，正常次月实发及申报后的缴款不作为关上月账的前置条件；已申报未缴保持步骤完成事实，同时按法定期限显示付款提醒。零笔未匹配只表示已导入的流水没有待匹配项，不表示银行资料已经齐全。用户直接提供具体任务或资料时，AI 选择公司后立即处理，不以等待全部流水为由插入无关看板或延迟事实已经完整的任务。

事实完整且处理唯一时，AI 直接发起正式工具调用，由 Codex 的写工具审批完成普通确认，不在聊天中重复询问“是否确认”。仍有会改变处理的关键事实时，AI 必须先用已有材料、流水、关联事件和冻结规则自行调查推理；能够形成一个有依据的最可能方案时，先给出包含相关字段的完整建议，只请负责人确认是否正确，不正确时负责人直接补充差异，不得把字段模板或调查工作交还负责人。只有现有证据不足以支持任何候选方案时，才说明原因并询问一个最小事实；建议未经明确确认不得写入，沉默不视为确认。关账密码、本地登录、预览哈希和其他专用确认仍按内核原有流程执行。AI 使用简洁中文报告业务结果，不使用固定称呼；默认不展示原始 JSON、科目借贷行或内部 UUID。
...
5. 固定待办第 2 项“员工及工资变动”只确认入职、离职、停薪、工资奖金、参保和缴费基数变化；确认写入追加式月度事实并绑定当前人员档案快照，不要求工资已经过账。第 3 项先用 `finance_preview_payroll_contribution_assessment` 形成社保公积金核定快照；外部实际数不同于政策数时先登记实际数，再确认申报核定事实，并以同一快照完成常规工资和单位缴费计提。已申报未缴可完成第 3 项，后续缴款独立提醒且不阻断上月关账。
6. 固定待办第 4 项“个人所得税”进入 `🔄` 时，只要当期存在纳入工资个税申报的已过账常规工资，AI 就主动调用 `finance_generate_payroll_tax_import`，不先询问是否需要生成。该工具要求幂等键，读取当月正式工资明细和逐人已确认导入事实，生成并校验税局“正常工资薪金所得”BIFF8 `.xls` 文件，同时在数据库追加保存工资及计算快照哈希、文件哈希、受控相对路径、行数和生成时间。新会话会重验并复用同哈希版本；工资、扣除或员工信息变化后旧版本失效，新版本不得覆盖不同内容的同名文件。AI 按哈希复制到当前用户桌面，只报告文件名、行数和导入核对动作，不在聊天中展示证件号码。文件生成不等于申报完成，只有负责人确认税务客户端提交结果并写入对应内核义务后该项才完成。
7. 工资发放及社保、公积金、个税缴纳通过受支持的业务事件核销正式开放项。工资批次冻结的付款日在关账时用于区分“尚未到期”和“已到期未结”；后者必须完成结算状态复核，但不把真实未付负债误判为漏记工资，也不强迫当月计提、次月发薪的企业提前付款。社保银行扣款同时包含滞纳金时，`social_insurance_payment` 只接受有银行流水和证据支持的 `details.social_insurance_late_fee_fen`，社保本金必须精确核销开放项，滞纳金固定计入“营业外支出—社保滞纳金”，调用方不能自选科目。
