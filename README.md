...
2. `finance_preview_accounting_period_close` 只读重算本月凭证、账户发生额、固定资产折旧、无形资产摊销、借款计息、工资批次及人工复核计数，并返回 SHA-256 计算哈希；同时在 `data.assistant_review_checklist` 返回面向 AI 的月末建议清单，区分“系统已完成”“发现待处理”和“必须由负责人确认”。
...
5. 提交关闭前，AI 必须逐项展示建议清单中所有非完成项，主动核对未交材料、银行账户、应收应付、资产启用和折旧、新入职与工资、社保公积金个税、发票税额与外部申报、借款及股东往来；不得把数据库空记录推断成没有业务。
6. 一期不支持反结账。关闭月原事实和凭证不改；错误只能在后续已生成开放月通过关联冲正及原专用工作流重记。
