...
`AGENTS.md` 负责在“仓库开发”和“真实会计操作”之间路由，不复制具体会计规则。真实会计操作由仓库级 `$accounting-operator` Skill 组织会话；内核面向业务 AI 的正式契约仍集中定义在 `src/ai_accounting/agent_contract.py`，按版本维护，并通过两条运行路径自动提供：MCP 初始化时的服务指令，以及所有 AI 应先调用的 `finance_get_event_schema.agent_operating_protocol`。月结建议清单也引用同一契约文本，避免多处独立维护造成语义漂移。

契约定义 AI 的身份、交流方式、确认边界和证据优先工作流。AI 在提问前充分读取并交叉核对用户已经提供的原始材料、规范化数据、银行流水和内核现有事实；能够唯一确定时直接使用受控工具推进。只有未知事实会改变金额、分类、归属期间、税额或能否入账时才提出一个最小、具体的问题，并说明已核对事实、当前结论、准确缺口及其影响。AI 不得用“还有没有收入／费用”等泛泛问题代替材料审阅，不得把数据库空记录当作没有业务，也不得臆测缺失事实。材料全部核对后，完整性兜底问题必须明确限定为“除已提供并核对的材料外，是否另有尚未提供且会影响本次记账或报税的业务材料”。
...
只说“开始记账”时，AI 先调用只读 `finance_get_owner_brief` 获取背景，再调用 `finance_get_owner_workflow` 读取内核派生的权威九项状态。摘要只表示内核已经保存的事实，不能据此宣称没有其他业务；清单的符号、期限、完成依据、当前动作和关账门禁只能采用流程工具返回值，不能由聊天记忆或 AI 自行打勾。老板对“无人员变化”“材料已齐”“已申报”等事实作答后，AI 必须调用对应的类型化确认工具持久化，并重新读取流程；相关员工、工资、政策、实际申报或材料快照发生变化时，旧确认自动过期并重新打开步骤。通用开场使用“当前处理”展示当前动作和完整 `queue_steps`；只有当前节点明确完成并正式切换到另一节点时才使用“下一步”，且其正下方必须附更新后的清单。同一节点内的核对、追问、等待和普通沟通不重复输出“下一步”或清单，负责人主动查询进度时仍可查看清单。第 1—6 项固定出现且只对应当前会计期间，期间一经关闭即成为这些月结步骤的终态依据，不因后续版本新增确认表或回放时缺少冗余流程事实而重新打开；第 6 项仍可准确显示关账备份失败。第 7—9 项只在确有待办、到期、逾期或等待关账的义务时出现，已完成或当前不适用时隐藏；条件项仍保留原编号，不重新编号。每行只显示一个状态符号：`✅` 已完成、`🔄` 当前、`⏰` 到期或逾期、`⬜` 待办、`➖` 本期无。固定顺序为：银行流水；员工及工资变动；社保及公积金；个人所得税；票据及非银行业务；关账确认；税费申报及财务报表；企业所得税年度汇算清缴；工商年报。第 4 项只生成和确认当前期间的工资个税义务，不扫描或补问已关闭月份。第 7 项依赖第 6 项关账，不再反向阻塞关账；年度汇算和工商年报一旦产生会跨月持续显示，直至确认完成。负责人确认第 7—9 项适用历史义务已完成、但实际完成日期没有留存时，AI 使用流程返回的截止候选调用专用历史追认工具，保存“负责人确认完成、具体日期未建立”的类型化事实。工作流同时返回机器可读的 `confirmation_targets`；自然语言指向一个还是多个事项由 AI 根据当前公司、正在处理的清单项和对话上下文理解，内核只接受自己生成的义务 ID 和快照。相邻步骤不得合并成一个大问题，只列负责人需要提供、确认或在外部办理的事项。上月工资和单位社保公积金计提属于关账义务；社保公积金和个人所得税在关账流程前完成申报并核对申报值。正常次月实发及申报后的实际缴款不作为关上月账的前置条件，也不要求负责人提供缴款状态或日期；付款在实际发生月份由银行流水和类型化付款事件核销。零笔未匹配只表示已导入的流水没有待匹配项，不表示银行资料已经齐全。用户直接提供具体任务或资料时，AI 选择公司后立即处理，不以等待全部流水为由插入无关看板或延迟事实已经完整的任务。

事实完整且处理唯一时，AI 直接发起正式工具调用，由 Codex 的写工具审批完成普通确认，不在聊天中重复询问“是否确认”。仍有会改变处理的关键事实时，AI 必须先用已有材料、流水、关联事件和冻结规则自行调查推理；能够形成一个有依据的最可能方案时，先给出包含相关字段的完整建议，只请负责人确认是否正确，不正确时负责人直接补充差异，不得把字段模板或调查工作交还负责人。只有现有证据不足以支持任何候选方案时，才说明原因并询问一个最小事实；建议未经明确确认不得写入，沉默不视为确认。关账密码、本地登录、预览哈希和其他专用确认仍按内核原有流程执行。当前操作若因工具错误、文件冲突、审批拒绝、认证或服务故障而中断，且必须先解除该阻断，本次只报告未完成状态、错误原因、一个恢复动作和稳定错误码，不输出“下一步”或待办清单；故障解除并成功继续后才恢复正常清单。业务事实缺失的 `needs_information` 不自动视为此类技术错误。AI 使用简洁中文报告业务结果，不使用固定称呼；默认不展示原始 JSON、科目借贷行或内部 UUID。
...
- 幂等入账、未关账月份直接修改及删除、期间关闭校验、关联冲正、凭证规则轨迹和审计日志。
- [代收代付](docs/pass-through-funds.md)：混合收款拆分应收核销与代收应付款，按实际债权人代付或归还已明确的代垫款。
...
- 面向 Codex 新会话的仓库级会计执行 Skill、只读负责人摘要工具，以及跨会话恢复九项状态、期限、完成证据、关账门禁和个税导出版本的权威负责人流程工具。
...
项目包含 `.codex/config.toml`，其启动命令和工作目录按本仓库固定路径 `D:\GitHub\ai-accounting-core` 配置；如果仓库位于其他路径，需要同步修改这两个值。数据库、证据目录和生产安全控制只从应用的 `.env` 读取，禁止在 MCP 配置中重复覆盖，否则可能把生产模式与开发数据库配置混用并导致服务在加载工具前退出。在 Codex 中信任并打开本仓库、确认 PostgreSQL 已启动和迁移完成后，重启 Codex 即可加载 `ai_accounting` MCP。Codex 官方配置说明见 [Model Context Protocol](https://learn.chatgpt.com/docs/extend/mcp?surface=cli)。

Codex 可以为同一项目的多个任务分别启动 STDIO MCP 进程；这些正常服务实例共享运行租约，可以同时处理会计任务。备份仍使用同一锁文件的独占租约：任一服务运行时不能执行整库备份，备份进行时也不能启动服务。该边界只隔离备份与在线服务，不要求为了普通记账关闭其他会计任务。
...
公司的位置和就绪状态。位置按公司独立配置，不由每次关账请求传入；每家公司在自己的目录
保留两个仅含该公司数据库及其证据的 `.finance-company.zip`：无 `previous` 的文件是当前版，
带 `.previous` 的文件是上一版。每次成功关账先验证并滚动保留旧的当前版，再原子发布新当前
版；生成失败时至少保留旧当前版。正式库启用后的初始备份也必须为每家公司独立生成并验证
`<统一社会信用代码>.finance-company.zip`；`create` 产生的 `.complete` 目录只是中间产物，不能
作为初始备份完成品。

1. `finance_generate_accounting_period` 从企业开始记账的任意过去月份起逐月连续生成；不能跳月，也不能生成 Asia/Shanghai 当前月之后的月份。
2. `finance_preview_accounting_period_close` 只读重算本月凭证、账户发生额和全部内核关账义务，并返回 SHA-256 计算哈希。关账义务按可证明程度分层：有规范事实即可确定的工资及单位社保公积金计提、固定资产折旧、无形资产摊销、借款利息、已启动劳务批次、银行对账、报表可生成性，以及与当前快照一致的人员复核、社保公积金核定和非银行材料完整性事实是硬门禁，未完成时内核直接拒绝关账；最终请求中的临时布尔值不能绕过这些持久化门禁。社保公积金和个人所得税先完成外部申报并核对申报值；工资、社保、公积金和个税的现金结算允许跨月且不直接阻塞关账。已经导入的付款流水继续受流水匹配和银行对账硬门禁约束。流程解析器把确认快照哈希纳入关账计算哈希，关账并完成自动备份后第 6 项才完成；季度税费和财务报表外部申报在关账后的第 7 项处理。年度企业所得税汇算从次年持续显示至完成，5 月 31 日后转为逾期；工商年报从次年 1 月 1 日起持续显示至完成，6 月 30 日后转为逾期。
...
4. `finance_confirm_accounting_period_close` 必须携带该 `owner_approval_id`、预览返回的 `management_commentary.context_hash` 及负责人已经审阅的经营解读原文，并在税期企业锁和月份锁内复算同一关账哈希及上下文哈希；系统阻断为零、全部适用人工复核事实全真且有确认说明和证据时，保存完整不可变快照、经营解读及生成上下文并单向关闭期间。常规工资和法定款项的实际支付日期不属于上月关账复核；其开放项等待后续银行流水核销。业务事务提交后，系统导出该公司的一致性快照并生成便携 ZIP，返回 `close_backup` 的文件名、摘要和状态。AI 不能代替负责人签发确认，也不能仅凭清单字段为真自动关账。
...
负责人明确把当前公司数据库指定为可丢弃、将由回放重建的测试库时，可临时调用
`finance_configure_historical_test_close_mode` 启用历史测试批量关账模式。该模式应按操作类型
分阶段处理全部可用月份：集中预检、批量补同类前置事项、连续调用
`finance_confirm_historical_test_period_close` 关账；专用关账返回
`close_backup.status=deferred`，期间不逐月生成备份或更新回放资料。批次完成或中止后必须立即
关闭该模式，再统一补回放与备份。普通正式库不得启用该模式，仍须逐次密码复核和自动备份。
...
2. 新设企业首个申报年度不是完整自然年时，先调用 `finance_confirm_financial_statement_opening_balance`，依据营业执照等成立资料明确确认成立时点期初余额为零；迁移中的存量企业不得使用该确认代替真实期初余额。
3. 调用 `finance_get_financial_statement_requirements`，逐项处理历史通用管理费用、销售费用和财务费用的明细分类。分类绑定具体凭证行，拆分合计必须等于该行金额；更正会追加新版本，不修改旧记录。
4. 当年年初（新设企业首年为成立月）至季度末的全部月份关闭后，调用 `finance_preview_quarterly_financial_statements`。结果包含三表、勾稽检查、来源结账哈希、模板版本和报表计算哈希；任何未映射非零损益或现金事件都会返回 `needs_information`。
5. 在本地财务看板的“财务报表”页核对三表后，通过页面下载已填充的电子税务局 Excel 导入文件 `财务报表报送与信息采集（小企业会计准则）月季报_YYYYQn.xlsx`；也可以使用同一只读预览与下载接口完成自动核对。导出器只复制内置固定模板并写入指定单元格，不在服务器落地临时申报文件。
6. 负责人仍需在电子税务局手工导入、核对并停在提交前验收；系统不会登录或提交申报。
...
- [国家税务总局：2026 年度纳税申报期限](https://fgk.chinatax.gov.cn/zcfgk/c102424/c5245729/content.html)
- [国家税务总局：个人所得税扣缴申报管理办法](https://www.chinatax.gov.cn/n810219/n810744/n3752930/n3752974/c3963396/content.html)
- [浙江税务：社会保险费申报缴费规则](https://zhejiang.chinatax.gov.cn/art/2023/8/10/art_13314_595952.html)
- [市场监管总局：企业年度报告期限规定](https://www.samr.gov.cn/cms_files/filemanager/samr/www/samrnew/samrgkml/nsjg/fgs/202203/W020220302478403609033.pdf)
