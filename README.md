...
只说“开始记账”时，AI 调用只读 `finance_get_owner_brief` 汇报公司、开放期间、未匹配流水、迟到银行证据、应收应付和最近入账；存在开放期间时还会读取银行范围、已导入流水和当前对账状态，并对已经结束的最早开放期间执行只读关账预览，以确定人员、法定缴费及周期申报状态。该摘要只表示内核已经保存的事实，始终返回外部资料完整性尚未建立，不能据此宣称没有其他业务。开场最后展示一个可立即执行的“下一步”和完整九项“老板待办”，排序只决定当前追问；九项从第一条回复起全部可见，不适用的周期事项标为“本期无”。固定顺序为：银行流水；员工及工资变动；社保及公积金申报缴款；个税全员全额扣缴申报缴款；票据及非银行业务；税费申报及财务报表；企业所得税年度汇算清缴；工商年报；关账确认。相邻步骤不得合并成一个大问题，只列负责人需要提供、确认或在外部办理的事项。上月工资计提属于关账义务，正常次月实发不作为关上月账的前置条件；次月流水出现或冻结到期日要求时再复核结算。零笔未匹配只表示已导入的流水没有待匹配项，不表示银行资料已经齐全。用户直接提供具体任务或资料时，AI 选择公司后立即处理，不以等待全部流水为由插入无关看板或延迟事实已经完整的任务。

事实完整且处理唯一时，AI 直接发起正式工具调用，由 Codex 的写工具审批完成普通确认，不在聊天中重复询问“是否确认”。关账密码、本地登录、预览哈希和其他专用确认仍按内核原有流程执行。AI 使用简洁中文报告业务结果，不使用固定称呼；默认不展示原始 JSON、科目借贷行或内部 UUID。
...
- 面向 Codex 新会话的仓库级会计执行 Skill，以及只汇总内核已知待办、不推断外部资料完整性的只读负责人摘要工具。
...
2. `finance_preview_accounting_period_close` 只读重算本月凭证、账户发生额和全部内核关账义务，并返回 SHA-256 计算哈希。关账义务按可证明程度分层：有规范事实即可确定的工资计提、固定资产折旧、无形资产摊销、借款利息、已启动劳务批次、银行对账和财务报表可生成性是硬门禁，未完成时内核直接拒绝关账；工资、社保、公积金和个税的现金结算允许跨月，已到期未结项会进入独立必审清单，但确实未付仍应保留为负债，不得为了关账虚构付款。已经导入的付款流水继续受流水匹配和银行对账硬门禁约束。`data.assistant_review_checklist` 区分“系统已完成”“发现待处理”“必须由负责人确认”和“本期尚未到期”，其中 `management_commentary` 还提供版本化的只读证据上下文、写作要求和 `context_hash`。季度末使用与报表导出相同的累计计算预检，只排除本次关账将自行满足的当前月关闭状态和快照。清单按企业申报周期触发：按季事项仅在 3、6、9、12 月检查；上年度企业所得税汇算清缴在 4 月月结时提醒，以便 5 月 31 日前办理；上年度工商年报在 5 月月结时提醒，以便 6 月 30 日前办理；其他按年事项在 12 月集中检查。未到期项目在九项队列中标为“本期无”，不得逐月提出确认问题。
...
6. 工资发放及社保、公积金、个税缴纳通过受支持的业务事件核销正式开放项。工资批次冻结的付款日在关账时用于区分“尚未到期”和“已到期未结”；后者必须完成结算状态复核，但不把真实未付负债误判为漏记工资，也不强迫当月计提、次月发薪的企业提前付款。社保银行扣款同时包含滞纳金时，`social_insurance_payment` 只接受有银行流水和证据支持的 `details.social_insurance_late_fee_fen`，社保本金必须精确核销开放项，滞纳金固定计入“营业外支出—社保滞纳金”，调用方不能自选科目。
