...
function result(name, marker) {
  ...
  if (name === "Funds") return { schema_version: 3, snapshot_version: "same", selected_period: { label: "一月" }, data: { marker, accounts: [], investments: { products: [] }, bank_statement: {} } };
...
test("actual API consumers reject stale schemas, missing page metadata and mismatched returned counts", async () => {
  ...
  const valid = { schema_version: 3, data: { period_preparation: preparation, employees: { items: [] }, collections: { employees: { items: [], page } } } };
  const rejected = [
  ...
  assert.equal((await requestJson("/api/dashboard/employees?period=2026-01")).schema_version, 3);
});
...
test("embedded historical settlement pages require their shared scope and accurate returned counts", async () => {
  ...
  assert.equal((await requestJson("/api/dashboard/employees?period=2026-11")).schema_version, 3);
});
