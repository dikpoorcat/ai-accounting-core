...
- [ ] 已确认使用隔离的本地 PostgreSQL 17 实例和试用所需的存储目录；不复用共享或生产环境。
...
- [ ] 已使用 `finance-bootstrap` 创建唯一的试用企业，并妥善保存输出的 `org_id`。
- [ ] 在录入任何正式业务前，已从开始记账月份连续生成所需的开放会计期间。
- [ ] 已创建唯一的本地负责人账号，妥善保存一次性恢复码，并完成 `finance-login` 登录。
- [ ] 已在登录后重启 Codex，并确认 MCP 使用已认证的负责人会话。
- [ ] 已由用户明确授权开始试用；未获授权前不导入真实业务资料或部署为生产服务。
