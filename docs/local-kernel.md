...
| `integrity.py`、`projections.py`、`maintenance.py`、`read_state.py` | 权威内容核验、四表投影、受控维修和读取修复计数 |
...
备份先用 Online Backup API 生成一致 SQLite 快照，再核验结构、外键、公司身份、证据、全部保留事实／计算／凭证、关账和派生数据。返回 `verification`；历史采用依据不足时保留 `limited` 及具体期间，不宣称完整核验。恢复回执保留同一限制，旧文件和旧 ZIP 不因核验而改写。
