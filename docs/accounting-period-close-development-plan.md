...
检查器只检查已存在且具有明确合同/卡片/批次事实的义务，不从“没有工资批次”等情况推断无义务，也不自动生成业务事件。未结清应收应付、未匹配银行流水、未确认税期和未提交工资事实可以跨期，本阶段只作为人工复核计数与警告，不自动当作系统阻断。月结后补导入属于已关闭月份的外部银行流水涉及单独的数据保留与体验取舍，按决策文档 DEC-013 暂停，不在本闭环中擅自固化。
