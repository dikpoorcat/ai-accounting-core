...
当前源码及运行包采用 `ai-accounting-kernel/2`，目录库和公司库均为 `draft / 0`，公司备份格式为 2。打包只携带新合同，不包含旧升级链或真实资料；软件 manifest 记录两类数据库的格式和指纹。开发库及备份只在精确结构相同时可用，不提供旧系统或不同开发结构转换。

当前包用于合成资料验收，第 9 阶段才冻结正式首版。仓库源码更新不等于独立运行包已更新，交付时须重新打包并验证。
...
包内验证使用临时合成公司，覆盖身份登记与纠错、银行／现金／公司平台的备用金支出和退款、工资确认及混合付款、银行对账、密码批准关账、闭期更正、迟到资料承接与备份恢复。工资缺确认或修订后方案未更新时必须拒绝发布；个税映射缺失只影响文件。验证报告记录三种渠道、工资全额与备用金部分、管理费用净额、迟到资料处理前后和冻结内容不变；不操作真实资料或已有服务。
