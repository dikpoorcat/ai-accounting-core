...
> 前置基线：工资模块第七轮验收已通过；本阶段不得削弱既有数据库、迁移、MCP、STDIO 和全量回归门禁
...
- A：专用 Schema、纯计算器、严格金额/月份/税额算法和单元/性质测试。
- B：四张规范表、系统科目、`0009` 迁移、提交点约束和 PostgreSQL 负向/并发/迁移测试。
- C：购置、启用、折旧、处置服务，开放项/银行流水、幂等、冲正、MCP 和 STDIO 测试。

`models.py`、`service.py`、`schemas.py`、`mcp_server.py` 和迁移同一时刻只允许一个写入负责人。公共字段、事件名、错误码、公式或科目角色变化必须先更新本文档并由总设计 Agent 裁决。
...
迁移、往返、污染预检和数据库提交点测试只允许显式连接唯一命名的临时 PostgreSQL 17。默认 Compose `finance` 数据库只读，不得升级、降级或写入。

最终必须独立确认：四类事件模板平衡；幂等与换载荷冲突；依赖逆序冲正；整数分末月闭合；处置月折旧；开放项不可超额；银行匹配可追踪；数据库直写不能制造跨企业、跳月、超折旧、重复处置或篡改正式事实；真实 STDIO Schema 不含自由科目/借贷字段且完整生命周期可复现。
