...
@pytest.mark.parametrize("dimension", ("profile", "policy", "opening"))
def test_r6_001_0006_preflight_rejects_historical_final_correction_pollution(
    dimension: str,
) -> None:
    """A real 0006 schema cannot be upgraded past final-version pollution.

    The rows below use only revision-compatible SQL.  No head column,
    temporary DDL, or current-schema mapper participates in this fixture.
    """

    with _postgres_at_0006() as (config, engine):
        ids = _historical_0006_final_regular(engine, key=f"preflight-{dimension}")
        with engine.begin() as connection:
            if dimension == "profile":
                connection.execute(
                    sa.text(
                        ...
                        "expense_role, social_insurance_base_fen, housing_fund_base_fen, "
                        "resident_employee, created_at) VALUES "
                        "(:id, :org_id, :employee_id, :profile_id, '2026-09-01', '2026-09-30', "
                        "'payroll_management_expense', 1000001, 1000001, TRUE, now())"
                    ),
                    {**ids, "id": uuid.uuid4()},
                )
                ...
                connection.execute(
                    sa.text(
                        "INSERT INTO payroll_policy_versions "
                        "(id, org_id, region, supersedes_id, effective_from, effective_to, "
                        "version, "
                        "source_url, parameters, created_at) "
                        "SELECT :id, :org_id, region, :policy_id, '2026-09-01', '2026-09-30', "
                        "'r6-0006-pollution', source_url, parameters, now() "
                        "FROM payroll_policy_versions WHERE id = :policy_id"
                    ),
                    {**ids, "id": uuid.uuid4()},
                )
                ...
                connection.execute(
                    sa.text(
                        ...
                        "cumulative_special_additional_deduction_fen, "
                        "cumulative_other_legal_deduction_fen, cumulative_tax_relief_fen, "
                        "cumulative_tax_withheld_fen, now() "
                        "FROM payroll_opening_states WHERE id = :opening_id"
                    ),
                    {**ids, "id": uuid.uuid4()},
                )
        _assert_0006_to_0007_preflight_refusal(
            config, engine, "R6_FINAL_PAYROLL_CORRECTION_PRECHECK_FAILED"
        )


def test_r6_004_0006_preflight_rejects_noncanonical_historical_evidence() -> None:
    """0006's length-only Evidence row must not be silently canonicalized."""
        ...
        with engine.begin() as connection:
            org_id = connection.execute(
                sa.text(
                    ...
                    "(:id, 'R6 0006 哈希污染', 'small_scale', 'quarterly', 'CN', "
                    "0.07, 'small_enterprise', now()) RETURNING id"
                ),
                {"id": uuid.uuid4()},
            ).scalar_one()
            connection.execute(
                sa.text(
                    ...
                    "(:id, :org_id, :sha256, 'legacy.txt', 'text/plain', 'legacy', 1, "
                    "'/legacy/evidence', '{}'::json, now())"
                ),
...
def test_r6_0007_downgrade_and_reupgrade_preserve_a_real_revision_boundary() -> None:
    ...
    with PostgresContainer("postgres:17-alpine@sha256:742f40ea20b9ff2ff31db5458d127452988a2164df9e17441e191f3b72252193", driver="psycopg") as postgres:  # noqa: E501
        ...
        command.upgrade(config, "head")
        command.check(config)
        command.downgrade(config, "0006_payroll_round5_integrity")
        command.upgrade(config, "head")
        command.check(config)
...
def test_r6_003_final_settlement_primary_key_is_immutable_but_service_reversal_is_legal(
    postgres_engine: object,
) -> None:
    """A PEL-proven settlement cannot change identity; canonical reversal can."""

    with Session(postgres_engine) as session:
        identifiers = _salary_payment_with_unsettled_statutory_sources(
            session, key="r6-settlement-id"
        )
        ...
        assert salary_event_id is not None and settlement_id is not None
        identifiers.update({"salary_event_id": salary_event_id, "settlement_id": settlement_id})
        session.commit()

    _commit_rejects(
        postgres_engine,
        sa.text("UPDATE settlements SET id = :replacement_id WHERE id = :settlement_id"),
