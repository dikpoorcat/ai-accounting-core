...
3. 新安装尚无负责人时，按明确安装授权请求 `kind="bootstrap_owner"`；已有目录身份异常不能通过重建负责人或数据库绕过。窗口取消时结束该登录尝试，不循环弹窗。
   启动只复用或恢复登录。单月关账批准在业务资料和同版月度核对准备好后请求，不在启动时提前弹出关账密码窗口；已有安全请求先查询持久状态，避免中断后重复要求密码。
...
日常启动不包含重新安装、清库、导入旧账、回放或正式账务写入。结构指纹、运行时或公司身份不匹配时报告实际错误，不能重新建表掩盖问题。当前默认连接独立的 `data/kernel-draft` 开发根，只接受新系统当前精确结构；旧系统、旧备份或不同开发结构直接拒绝。启动不执行结构升级，持久任务只在目录库确认有效后恢复。
